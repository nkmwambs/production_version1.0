...
class Partner extends CI_Controller
{
	...
	function budget_summary()
	{
		...
		//$page_data['month_totals'] = $this->finance_model->budget_spread_month_totals($fyr, $this->session->center_id);
		$page_data['months_range'] = [
	...
	function voucher_accounts($param1 = '', $voucher_item_type_id = 0)
	{
		...
		$allow_support_mode_and_recipient = $this->db->get_where(
			'voucher_type',
			array('voucher_type_abbrev' => $param1)
		)->row()->allow_support_mode_and_recipient;
	...
	function chqIntel($param1)
	{
		...
		$bank_code_obj = $this->db->get_where(
			'projectsdetails',
	...
	function post_voucher()
	{
		...
		$allow_support_mode_and_recipient = $this->db->get_where(
			'voucher_type',
			array('voucher_type_abbrev' => $this->input->post('VTypeMain'))
		)->row()->allow_support_mode_and_recipient;
	...
	function reverse_voucher($voucher_id, $reuse_cheque)
	{
		...
		$voucher = $this->db->get_where(
			'voucher_header',
			array('hID' => $voucher_id)
		)->row_array();
		...
		if ($this->db->trans_status() == false || $new_voucher_id == 0) {
	...
	function check_if_voucher_is_already_reversed($voucher)
	{
	...
	function insert_voucher_reversal_record($voucher, $reuse_cheque)
	{
		...
		// Check if the voucher is already reversed
		$already_reversed = $this->check_if_voucher_is_already_reversed($voucher);

		if (!$already_reversed) {
			//Unset the primary key field
			$voucher_id = array_shift($voucher);

			$voucher_body_details = $this->db->get_where(
				'voucher_body',
				array('hID' => $voucher_id)
			)->result_array();
	...
	function post_edit_variance_comment()
	{
		...
		$condition = array(
			'icpNo' => $this->session->center_id, 'AccNo' => $post['account_number'],
	...
	function submit_mfr($project_id, $date)
	{
		...
		if ($proof_of_cash <> 0 || $bank_validation <> 0 || $bs_check == 0) {
			...
			$this->finance_model->register_dashboard_change($this->session->center_id, $date);
